...
TO_DO: Convert to handle binned posterior.
...
v_pos = np.linspace(-cat.par['vcut'], cat.par['vcut'], par_proc['shape'][0] + 1)
a_pos = np.linspace(0, cat.par['aperture'], par_proc['shape'][1] + 1)
...
dtype = [('rockstarId', '<i8'), ('logmass', '<f4'), ('N', '<i4'), ('N1.0', '<i4'),
         ...
         ('logmass_keras','<f4'), ('logmass_keras1.0', '<f8'), ('err_keras', '<f4'),
         ('logmass_sigv','<f4'), ('logmass_sigv1.0', '<f8'), ('err_sigv', '<f4'),
         ('gen','?'),('mbin','?'),('nbin','?'),     
        ]
...
def sample_rich(i_proc):
    ...
    for i in range(max(multi[i_proc]) + 1):
        ...
        kdeval = np.reshape(kde(sample).T, [1] + list(mesh[0].shape) )
