...
pdfs = np.ndarray(shape = (len(dat), *par['shape']))

mesh = np.mgrid[-vmax : vmax : par['shape'][0]*1j,
                0 : rmax : par['shape'][1]*1j               
                ]

sample = np.vstack([mesh[0].ravel(), mesh[1].ravel()]) # Velocities at fixed intervals. Used to sample velocity pdfs

print('Generating ' + str(len(dat)) + ' KDEs...')
for i in range(len(dat)):
    if i%1000==0: print(str(int(i/1000)) + ' / '+str(int(len(dat)/1000)))
    memb = np.ndarray(shape=(2,dat['Ngal'][i]))
    
    memb[0,:] = dat['vlos'][i][:dat['Ngal'][i]]
    memb[1,:] = dat['Rproj'][i][:dat['Ngal'][i]]
