...
def build_config():
    ...
    config = OrderedDict([
        ...
        ('volume',          1000.**3), # (Mpc/h)^3  # 2000.**3 # #  352.**3), 302.6**3),# 
        ('dn_dlogm',        10.**-5.2), # -5.2
        ('dlogm',           0.01),
        ('boost_minmass',   10**13.),
        ('min_rotations',   3),
    ...
    from mock_config import new_config
...
def handle_parameters():
    ...
    elif config['cut_size'] == 'extralarge':
        config['aperture'] = 3.5  # Mpc/h
        config['vcut'] = 6000.  # km/s
    else:
...
def load_and_preprocess():
    ...
    print('\nLoading data...')
    host_data = load_raw(join(config['wdir'], config['in_folder'],
                                      config['host_file']))
...
#     host_data = host_data.rename(columns={'upid':'upId','id':'rockstarId'}, copy=False)
    ...
    if config['gal_file'] is None:
        ...
        gal_data = load_raw(join(config['wdir'], config['in_folder'],
                                         config['gal_file']))
...
#                                             'mvir': 'Mvir'}, copy=False)
    ...
    host_data = host_data[(host_data[config['host_upid_def']] == -1) &
                          (host_data[config['host_mass_def']] >= config['host_min_mass'])]

    gal_data = gal_data[(gal_data[config['gal_upid_def']] != -1) &
...
def find_true_members():
    global config, gal_data, host_members
    
    print('\nAssigning true members...')

    # Relates host host_id_def to indices of member galaxies
    ...
    for i in range(len(gal_data)):
        ...
        # Assign to host
        host_members[gal_data.loc[i, config['gal_upid_def']]].append(i)
...
def reassign_true(host_i):
    ...
    host_mem = host_members[host_data.at[host_i, config['host_id_def']]]

    for j in range(len(host_mem)):
        true_memb_dist = np.sqrt(np.sum(
            (gal_data.iloc[host_mem[j]][['x', 'y', 'z']] - host_pos)**2))
        if (true_memb_dist > host_data.at[host_i, config['host_R_def']] / 1000.) & \
...
def pad():
    ...
    for i in range(len(host_data)):
        ...
        host_members[host_data.at[i, config['host_id_def']]] = reassignments[i]
...
def get_rot_assign():
    ...
    host_rots.loc[:, 'logM'] = np.log10(host_data[config['host_mass_def']])
    ...
    min_cols = [(0, 0, 0), (np.pi / 2, 0, 0), (np.pi / 2, np.pi / 2, 0)]
    if config['min_rotations'] < 3:
        min_cols = min_cols[:config['min_rotations']]
...
def save_intermediates():
    ...
    with open(join(cache_dir, 'cache.p'), 'wb') as f:
        pickle.dump(config, f)
    with open(join(cache_dir, 'host_members.p'), 'wb') as f:
        pickle.dump(dict(host_members), f)
    
    host_data.to_pickle(join(cache_dir, 'host_data.p'))
    gal_data.to_pickle(join(cache_dir, 'gal_data.p'))
    
    if config['split_rot']<=1:
        rot_assign.to_pickle(join(cache_dir, 'rot_assign.p'))
    else:
        ...
        for i in range(config['split_rot']-1):
            ...
            x.to_pickle(join(cache_dir, f'rot_assign_{i}.p'))
        
        x = rot_assign.iloc[:, j:]
        x.to_pickle(join(cache_dir, f'rot_assign_{i+1}.p'))
...
def load_intermediates():
    ...
    with open(join(cache_dir, 'cache.p'), 'rb') as f:
        config = pickle.load(f)
    with open(join(cache_dir, 'host_members.p'), 'rb') as f:
        host_members = pickle.load(f)
    
    host_data = pd.read_pickle(join(cache_dir, 'host_data.p'))
    gal_data = pd.read_pickle(join(cache_dir, 'gal_data.p'))
    
    if load_ind==None:
        rot_assign = pd.read_pickle(join(cache_dir, 'rot_assign.p'))
    else:
        rot_assign = pd.read_pickle(join(cache_dir, f'rot_assign_{load_ind}.p'))
...
#     else:
#         gal_prop += [(config['gal_id_def'], '<i8')]# , ('Vmax', '<f8'), ('Vpeak', '<f8')]
        
#     gal_prop = [('m', '<f8'), ('id', '<i8')]
    
    gal_prop = [(x, '<f8') for x in gal_data.columns.values
                if x not in ['x','y','z','vx','vy','vz']]
    ...
    for i in range(len(cl_list)):  # for each cluster
        ...
        try:
            mem_true_ind = host_members[host_data.loc[ind, config['host_id_def']]]
        except KeyError:
            print('error:',ind, host_data.loc[ind, config['host_id_def']], host_data.columns.values)
            continue
...
def consolidate_catalogs(catalogs, ang_seq):
    ...
    contam_catalog = Catalog(
        ...
        prop=pd.DataFrame(np.zeros(
            shape=(contam_len, catalogs[0][1].prop.shape[1] + 3)),
            columns=np.append(catalogs[0][1].prop.columns.values, ['rot_theta', 'rot_phi', 'rotation'])),
        gal=[None] * contam_len)
    ...
    pure_catalog = Catalog(
        par=cat_par.copy(),
        prop=pd.DataFrame(np.zeros(
            shape=(pure_len, catalogs[0][0].prop.shape[1] + 3)),
            columns=np.append(host_data.columns.values, ['rot_theta', 'rot_phi', 'rotation'])),
        gal=[None] * pure_len)
    ...
    for i in range(len(catalogs)):
        ang = ang_seq[i]
        if ang[:2] in rot_id_dict.keys(): # ang[-1]==0:
        ...
        pure_catalog.prop.iloc[pure_c:pure_c_end, :-3] = pure.prop.values
        pure_catalog.prop.loc[pure_c:pure_c_end, 
                              ['rot_theta','rot_phi']] = ang[:2]
        pure_catalog.prop.loc[pure_c:pure_c_end, 
                              'rotation'] = rot_num
        pure_catalog.gal[pure_c:pure_c_end] = pure.gal

        contam_catalog.prop.iloc[contam_c:contam_c_end, :-3] = contam.prop.values
...
def save_data(pure_catalog, contam_catalog):
    ...
    if not debug:
        ...
        if load_ind is None:
            pure_catalog.save(join(config['wdir'],
                                           config['out_folder'],
                                           config['catalog_name'] + '_pure.p'))
            contam_catalog.save(join(config['wdir'],
                                             config['out_folder'],
