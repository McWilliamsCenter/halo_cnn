...
~~~ THIS IS FOR EVAN TUCKER ~~~
Inclues:
		Larger aperture
		Less clusters
		Specifically-chosen clusters
...
print(np.isin(host_data['rockstarId'],cool_hosts))
if ~np.isin(host_data['rockstarId'],cool_hosts).any():
...
print('cool_hosts length: ' + str(len(host_data)))
...
def cut_mock( angle = None ):
    ...
    gal_rot['id'] = gal_data['id'].values
    gal_rot['upid'] = gal_data['upid'].values

    print('discrepancies:',np.argwhere(~(gal_rot['id'] == gal_data['id'].values)))
    ...
    for i in rot_assign.index.values[rot_assign[angle]]: # for each cluster
        ...
        if len(mem_proj_ind) > par['min_richness']:
            ...
            clu_gals['id'] = gal_rot.loc[mem_proj_ind,'id']
            clu_gals['upid'] = gal_rot.loc[mem_proj_ind,'upid']
            ...
            print(clu_gals['id'] == gal_rot.loc[mem_proj_ind,'id'])
        ...
        if len(mem_true_ind) > par['min_richness']:
            ...
            #                                       host_rot.loc[i,['x','y','z']]
            clu_gals['id'] = gal_rot.loc[mem_true_ind,'id']
            clu_gals['upid'] = gal_rot.loc[mem_true_ind,'upid']
...
if n_proc>1:
    ...
    catalogs = list(map(cut_mock, rot_assign.columns.values))
...
if not debug:
    ...
    pure_catalog.save_npy(os.path.join(par['wdir'], 
                                   par['out_folder'], 
                                   par['catalog_name'] + '_pure.npy'))
    contam_catalog.save_npy(os.path.join(par['wdir'], 
                                     par['out_folder'], 
                                     par['catalog_name'] + '_contam.npy'))
