...
for i in range(par['nfolds']):
	...
	x_test = [[(j,) for j in np.abs(cat.gal[m]['vlos'])] for m in test_ind]
	...
	if i<3:
		...
		plt.plot( y_train, np.log10([np.sum(np.array(a)**2) for a in x_train]), '.', label='train')

		plt.plot( y_test, np.log10([np.sum(np.array(a)**2) for a in x_test]), '.', label='test')
		    
		plt.xlabel('logM',fontsize=12)
		plt.ylabel('log sigv',fontsize=12)
		plt.legend()
		plt.tight_layout()
		f.savefig('fold_'+str(i)+'.pdf')"""
